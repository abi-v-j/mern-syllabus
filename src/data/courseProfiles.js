const courseProfiles = {
  ...
  html: {
    ...
    realLifeScenario:
      'you are building a restaurant landing page with headings, images, menus, and a contact form',
    realLifeAction:
  ...
  css: {
    ...
    analogy:
      'choosing the paint, furniture spacing, and lighting after the house structure already exists',
    realLifeScenario:
      'you want a dashboard or landing page to look clean on both desktop and mobile screens',
    realLifeAction:
      'you arrange cards, align sections, choose spacing, and make the design responsive',
    useCase: 'you can turn plain markup into interfaces that feel professional and easy to use',
  },
  'javascript-basics': {
    ...
    useCase: 'you can move from static pages to interfaces that actually respond to users',
  },
  'javascript-advanced': {
    simpleMeaning:
      'how to work with asynchronous code, browser APIs, and modern JavaScript patterns',
    analogy:
      'coordinating several moving parts in the background while the user keeps using the page normally',
    realLifeScenario:
      'you are loading live order updates, saving data locally, or calling an external API',
    realLifeAction:
  ...
  'git-and-github': {
    ...
    analogy:
      'keeping a timeline of every change so you can review, share, or roll back work confidently',
    realLifeScenario:
      'two developers are working on the same project and both need to ship updates this week',
    realLifeAction:
      'changes must be committed, pushed, reviewed, and merged without overwriting each other',
    useCase: 'your team can work faster without losing code or guessing what changed',
  },
  'react-js-basics': {
    ...
    useCase: 'you can build cleaner frontends that are easier to maintain as the app grows',
  },
  'react-js-advanced': {
    ...
    realLifeScenario:
      'you are building a multi-page dashboard with protected areas, lazy loading, and shared logic',
    realLifeAction:
      'pages need routing, side effects, reusable hooks, and performance-aware rendering',
    useCase: 'you can handle larger React apps without turning them into tangled component trees',
  },
  'state-management': {
    simpleMeaning:
      'how to decide where shared data should live and how different components update it',
    analogy:
      'choosing whether a note stays on one desk or is pinned to a board the whole team can see',
    realLifeScenario:
      'a cart count, logged-in user, filters, or theme needs to be used across many screens',
    realLifeAction:
      'several components must read the same data and stay in sync when one part changes it',
    useCase: 'your app becomes easier to reason about and much less repetitive',
  },
  'ui-development': {
    simpleMeaning: 'how to create consistent, reusable, and responsive interface patterns',
    analogy:
      'building a design toolkit so you do not redesign every button, form, or card from scratch',
    realLifeScenario:
      'you are designing an admin dashboard, marketing page, and data table in the same product',
    realLifeAction:
      'the interface needs to feel consistent even though it contains many different screens',
    useCase: 'you can build polished UI faster and keep the product visually coherent',
  },
  'node-js': {
    simpleMeaning: 'how JavaScript runs outside the browser to power servers, scripts, and tooling',
    analogy:
      'moving the same language from the front desk to the back office where the work is processed',
    realLifeScenario:
      'you need a server to read files, handle environment variables, or run a backend process',
    realLifeAction:
      'code now works with the operating system, packages, streams, and server-side tasks',
    useCase: 'you can use JavaScript across both the frontend and backend of a MERN project',
  },
  'express-js': {
    simpleMeaning: 'how to create routes and middleware for handling HTTP requests cleanly',
    analogy:
      'setting up reception desks that know where each request should go and what checks happen first',
    realLifeScenario:
      'your React app needs endpoints for products, login, orders, or profile updates',
    realLifeAction:
      'incoming requests must be validated, routed, processed, and answered consistently',
    useCase: 'your backend becomes easier to organize, debug, and extend',
  },
  mongodb: {
    simpleMeaning:
      'how to store application data as flexible documents instead of strict table rows',
    analogy:
      'keeping information in structured folders where each record can hold slightly different details',
    ...
    useCase: 'you can model real app data in a format that fits naturally with JavaScript objects',
  },
  mongoose: {
    simpleMeaning: 'how to add schema rules and cleaner database operations on top of MongoDB',
    analogy:
      'placing quality rules and helper tools on each folder before it is allowed into storage',
    realLifeScenario:
      'your app saves user records, products, or bookings and needs validation before writing them',
    realLifeAction:
      'you want safer CRUD logic, relationships, middleware hooks, and consistent timestamps',
    useCase: 'you reduce database bugs and keep model logic more maintainable',
  },
  'rest-api-development': {
    ...
    realLifeScenario:
      'a product listing screen needs search, filters, pagination, and sorting from an API',
    realLifeAction:
  ...
  'authentication-and-authorization': {
    ...
    realLifeScenario:
      'a user signs in to a dashboard and only admins should see management features',
    realLifeAction:
      'passwords, tokens, roles, and protected routes all need to work together correctly',
    useCase: 'you can secure sensitive parts of the app without blocking valid users',
  },
  security: {
    simpleMeaning: 'how to protect your app from common attacks and unsafe defaults',
    analogy:
      'locking the doors, checking deliveries, and adding alarms before opening a shop to the public',
    realLifeScenario:
      'your app accepts logins, form input, uploaded files, and browser requests from many users',
    realLifeAction:
      'bad input, abuse, or unsafe tokens can break the app or expose private data if you do nothing',
    useCase: 'you can ship features with safer defaults and fewer production surprises',
  },
  'file-upload': {
    simpleMeaning: 'how to accept and store user-uploaded files safely',
    analogy:
      'receiving packages at a warehouse, checking them, and sending them to the right shelf',
    ...
    useCase: 'you can add media features without making the app messy or unsafe',
  },
  'full-stack-integration': {
    ...
    realLifeScenario:
      'a React form sends data to an Express API and the page shows loading, success, or error states',
    realLifeAction:
      'requests, responses, auth tokens, and UI feedback all need to stay coordinated',
    useCase: 'your product feels reliable instead of disconnected or confusing',
  },
  'admin-panel-development': {
    ...
    realLifeScenario:
      'staff need to manage users, products, reports, and settings from one secure area',
    realLifeAction:
      'the layout, permissions, filters, and actions must stay efficient and easy to scan',
    useCase: 'you can support real operations, not just public marketing pages',
  },
  'payment-integration': {
    ...
    realLifeScenario:
      'a customer pays for an online order and expects the app to confirm the purchase immediately',
    realLifeAction:
      'orders, payment providers, verification, and invoices all need to stay in sync',
    useCase: 'you can connect revenue-critical features without guesswork',
  },
  'real-time-features': {
    ...
    realLifeScenario:
      'a chat window, live order tracker, or online/offline indicator updates while the user is watching',
    realLifeAction: 'the server pushes events and every connected client reacts right away',
    useCase: 'you can build interfaces that feel alive and timely',
  },
  testing: {
    ...
    realLifeAction: 'tests confirm behavior, catch regressions, and reduce fear during refactors',
    useCase: 'you can change code faster with more confidence',
  },
  deployment: {
    ...
    realLifeScenario:
      'you finished a project locally and now need it running on Vercel, Render, or another platform',
    realLifeAction:
      'builds, environment variables, databases, and hosting settings must all line up correctly',
    useCase: 'your project becomes shareable, testable, and ready for real users',
  },
  'mern-project-structure': {
    ...
    realLifeScenario:
      'your app now has controllers, services, components, pages, and shared utilities',
    realLifeAction:
      'without structure, even simple changes become slow because nobody knows where things belong',
    useCase: 'you can scale the codebase without turning navigation and debugging into a headache',
  },
  'advanced-mern-concepts': {
    simpleMeaning: 'how larger systems handle scale, performance, tooling, and operations',
    analogy:
      'upgrading from one small shop to a business with multiple teams, warehouses, and monitoring screens',
    realLifeScenario:
      'traffic grows, jobs move to the background, and one app becomes several cooperating parts',
    realLifeAction: 'you need caching, queues, logging, monitoring, and better system boundaries',
    useCase: 'you can reason about production systems beyond the basic CRUD stage',
  },
  'major-projects': {
    ...
    realLifeScenario:
      'you are planning a Todo app, ecommerce platform, chat app, or SaaS dashboard for a portfolio',
    realLifeAction:
      'features, data flow, UI, API design, auth, and deployment all need to come together',
    useCase: 'you build portfolio work that proves you can connect the full stack',
  },
  'interview-preparation': {
    ...
    realLifeScenario:
      'you are asked to explain a topic, defend a design choice, or talk through a project you built',
    realLifeAction:
